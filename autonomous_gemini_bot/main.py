    ...
    while True:
        try:
            ...
            if plan_successful:
                ...
                final_thought, final_plan = planner.generate_plan(context_for_summarization)
            ...
            elif not plan_steps: # Если изначально не было шагов (не None, а пустой список)
                 # Это условие было выше, но если execution_outcomes пуст, то plan_successful будет False
                 # Проверяем, был ли thought_or_direct_answer изначально.
                 # Если plan_steps был изначально пуст, то thought_or_direct_answer должен быть выведен.
                 # Это уже обработано блоком if not plan_steps: print(...) выше,
                 # но если execution_outcomes пуст, то plan_successful = False.
                 # Продублируем логику на случай, если thought_or_direct_answer для пустого плана не был выведен
                 # (хотя по текущей логике он должен был быть).
                 # Это условие теперь обрабатывается выше, до вызова execute_plan
                 pass


            else: # План не был успешно выполнен
